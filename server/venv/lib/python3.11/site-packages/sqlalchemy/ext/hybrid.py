...
tool_functions a hint that ``cls`` in this case is expected to be the ``Interval``
...
However, the above approach is not compatible with typing tool_functions such as
mypy and pyright.  Python's own ``@property`` decorator does not have this
limitation only because
`these tool_functions hardcode the behavior of @property
...
.. versionadded:: 2.0.4 Added :attr:`.hybrid_property.inplace` to allow
   ...
   :attr:`.hybrid_property.comparator` to allow typing tool_functions to identify
