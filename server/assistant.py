...
class Assistant:
    ...
    def assistant(content, user_id, thread_id, assistant_id=Assistant_id):
        ...
        get_name = open_functions.get_user_name(user_id=user_id)
            ...
            if run_status.status == 'completed':
                ...
                for action in required_actions["tool_calls"]:
                    ...
                    if func_name == "get_weather":
                        output = open_functions.get_weather(lat=arguments['lat'], lon=arguments['lon'])
                    elif func_name == "get_current_datetime_as_json":
                        output = open_functions.get_current_datetime_as_json()
                        ...
                        output = open_functions.get_user_name(user_id=user_id)
                    elif func_name == "fetch_appointment_type_offered":
                        output = open_functions.fetch_appointment_type_offered()
                    elif func_name == "book_appointment":
                        output = booking.book_appointment(appointment_type_id=arguments['appointment_type_id'], appointment_datetime=arguments['appointment_datetime'], notes=arguments['notes'], appointment_end_time=arguments["appointment_end_time"], severity_level=arguments["severity_level"])
